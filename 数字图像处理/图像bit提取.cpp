...
int main(void)
{
	...
	{
		ppm2 = ppm;
		ppm2.data = (UC *)malloc(ppm.height 
			* ppm.width * ppm.byte * sizeof(UC));

		for (i=0; i<8; i++)		// 提取八个比特图像
		{
			extract_bit(&tmp, &ppm, i);		// 提取第 i 比特
			update_bit(&ppm2, &tmp, i);		// 填入第 i 比特
			sprintf(path, "bit_%d.ppm", i);
			save_img(&tmp, path);
		}
		save_img(&ppm2, "合成后.ppm");
